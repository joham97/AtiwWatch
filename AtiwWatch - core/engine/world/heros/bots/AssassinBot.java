...
public class AssassinBot extends Assassin {

	private List<OtherPlayer> otherPlayers;

	public AssassinBot(World world) {
		super(world);
	}
		...
		if (nextEnemy != null && new FinallySmartVectors(nextEnemy.x - this.x, nextEnemy.y - this.y).len() < 375) {
			if (!this.world.getWalls().doesCollides(new MyLine(this.x, this.y, nextEnemy.x, nextEnemy.y))
				...
				if(canDoSpecialAttack()){
					FinallySmartVectors fsv = new FinallySmartVectors((nextEnemy.x-this.x), (nextEnemy.y-this.y)).unit();
					float dashToX = nextEnemy.x + fsv.x * 50;
					float dashToY = nextEnemy.y + fsv.y * 50;
					dash(dashToX, dashToY, delta);
				}
			}
		}
